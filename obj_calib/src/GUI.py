...
from tf.transformations import quaternion_from_matrix as rot2quat
...
from allign_3D_cloud import SVD, rot2eul, allign_3D_points
...
class UI():
    ...
    def __init__(self, points_on_model = [], node_name="obj_calib"):
        ...
        self.markerDisplay = markerDisplay(topic_name="points_array", node_name=node_name, color=(0, 0, 255, 255))        # marker display
        self.markerDisplay_model = markerDisplay(topic_name="model_points", node_name=node_name, color=(255, 0, 0, 255))  # marker model display
        self.meshDisplay = meshDisplay(node_name=node_name, topic_name="ref_model")            # Model display
    ...
    def fcalibrate(self):
        ...
        points_allign = [[0.6439, -0.2308, -0.0565], [0.5530, -0.0727, -0.0495], [0.7514, 0.0325, -0.0051], [0.8486, -0.1210, -0.0082]]
        ...
        trans[0] = trans[0] #+ 0.5*0.17
        quat = rot2quat(Rm)
        self.meshDisplay_cal = meshDisplay(topic_name="cal_model")            # Model display
        self.meshDisplay_cal.draw_mesh(pose = trans, rot = quat)  
        
        #transt = []
        #for i in trans:
        #    transt.append(-i)
        #alligned_points = allign_3D_points(self.points_on_model_og, R, transt)
        self.markerDisplay_model.draw_markers(list(points_allign))
...
if __name__ == "__main__":
    try:
        ...
        path = wd +  "/meshes/ipad6fix.stl"
        print(path)
        points_on_model = get_ponts(str(path))
        print("[INFO] Using calculated points")
    except:
        print("[INFO] Using hand picked points")
        points_on_model = [[0.1695/2, -0.24/2, 0.007], [-0.1695/2, -0.24/2, 0.007], [-0.1695/2, 0.24/2, 0.007], [0.1695/2, 0.24/2, 0.007]] # test for progressbar need length
        pass
